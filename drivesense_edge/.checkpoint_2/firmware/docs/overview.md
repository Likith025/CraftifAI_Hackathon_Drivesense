...
This ESP-IDF firmware reads the acceleration and gyroscope axes from the MPU-compatible IMU on an HW290 10DOF sensor and silently calibrates the gyroscope at startup, applies gyro-bias correction and low-pass filtering, calculates acceleration and angular-velocity magnitudes, and prints the filtered values every 1 second.
...
- `firmware/configs/app_config.h`: application pins, address, speed, sample period, calibration count, and filter coefficient.
