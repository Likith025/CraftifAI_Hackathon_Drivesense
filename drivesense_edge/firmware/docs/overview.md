...
This ESP-IDF firmware reads the acceleration and gyroscope axes from the MPU-compatible IMU on an HW290 10DOF sensor and silently calibrates the gyroscope at startup, applies gyro-bias correction and low-pass filtering, calculates acceleration and angular-velocity magnitudes, detects significant combined accelerometer/gyroscope motion with configurable hysteresis, evaluates motion every 100 ms with configurable pattern detection for harsh acceleration/braking, aggressive cornering, and road impacts, then prints the filtered values, magnitudes, and one prioritized status every 1 second.
...
- `firmware/configs/app_config.h`: application pins, address, speed, sample period, calibration count, filter coefficient, and motion thresholds.
